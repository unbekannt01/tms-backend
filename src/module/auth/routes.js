...
const { authLimiter, sensitiveOperationsLimiter } = require("../../middleware/rateLimiter")
const adminAuthMiddleware = require("../../middleware/adminAuthMiddleware")
const { resendEmailVerification } = require("./controllers/resendEmailVerification.controller")
...
router.post("/reset-password-with-backupCode", sensitiveOperationsLimiter, resetPasswordWithBackupCode)
router.get("/v2/verifyEmail/:token", emailVerifyToken)

router.post("/setSecurityQuestions", sensitiveOperationsLimiter, setSecurityQuestions)
router.post("/verifySecurityAnswers", sensitiveOperationsLimiter, verifySecurityAnswers)
router.get("/getSecurityQuestions", getSecurityQuestions)
...
router.post("/complete-security-setup", sessionAuthMiddleware, sensitiveOperationsLimiter, completeSecuritySetup)

// Session-protected routes
router.post("/users/logout", sessionAuthMiddleware, logOutUser)
router.get("/auth/me", sessionAuthMiddleware, getCurrentUser)
router.post("/change-password", sessionAuthMiddleware, sensitiveOperationsLimiter, changePassword)
...
router.post("/admin/sessions/logout-all", sessionAuthMiddleware, adminAuthMiddleware, adminLogoutAllSessions)

router.post("/admin/sessions/logout-user/:userId", sessionAuthMiddleware, adminAuthMiddleware, adminLogoutUserSessions)
