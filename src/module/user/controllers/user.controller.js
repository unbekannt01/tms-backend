...
const createUser = async (req, res) => {
  try {
    ...
    try {
      const welcomeEmailResult = await emailService.sendWelcomeEmail(savedUser, currentUser)
    ...
    if (currentUser && currentUser._id.toString() !== savedUser._id.toString()) {
      try {
        const notificationResult = await emailService.sendAccountCreationNotification(savedUser, currentUser)
        if (notificationResult.success) {
          console.log(`[v0] Account creation notification sent to manager: ${currentUser.email}`)
        }
      } catch (emailError) {
        console.error("[v0] Failed to send account creation notification:", emailError)
      }
    }
  ...
  try {
    ...
    for (const userData of usersData) {
      ...
      try {
        ...
        console.error(`[v0] Failed to send welcome email to ${savedUser.email}:`, emailError)
      }
    ...
    if (currentUser && savedUsers.length > 0) {
      try {
        const bulkNotificationResult = await emailService.sendAccountCreationNotification(
          {
        ...
        if (bulkNotificationResult.success) {
          console.log(`[v0] Bulk account creation notification sent to manager: ${currentUser.email}`)
        }
      } catch (emailError) {
        console.error("[v0] Failed to send bulk account creation notification:", emailError)
      }
    }
...
const createUserWithEmailToken = async (req, res) => {
  try {
    ...
    await emailServiceForToken.sendTokenEmail(savedUser.email, token, savedUser.firstName)
...
const getAllUser = async (req, res) => {
  try {
    ...
    if (search) {
      filter.$or = [{ email: { $regex: search, $options: "i" } }, { userName: { $regex: search, $options: "i" } }]
    }

    const adminRole = await Role.findOne({ name: "admin" })
    if (adminRole) {
      filter.roleId = filter.roleId ? { $eq: filter.roleId, $ne: adminRole._id } : { $ne: adminRole._id }
    }

    const users = await User.find(filter).populate("roleId", "name displayName").skip(skip).limit(limit)
...
const softDeleteUser = async (req, res) => {
  try {
    ...
    res.json({
      message: "User soft deleted. Will be permanently deleted after 30 days unless restored.",
    })
  } catch (error) {
