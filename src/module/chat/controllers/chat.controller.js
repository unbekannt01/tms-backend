...
const ensureConversation = async (userId, otherUserId) => {
  ...
  try {
    // Method 1: Use findOneAndUpdate with upsert (atomic operation)
    ...
    // Fallback: If there's still a race condition, find the existing conversation
    if (error.code === 11000) {
      const existingConvo = await Conversation.findOne({ participants: ids });
      if (existingConvo) {
...
const getOrCreateConversation = async (req, res) => {
  try {
    ...
    await convo.populate(
      "participants",
      "firstName lastName userName avatar roleId"
    );
...
const sendMessage = async (req, res) => {
  try {
    ...
    const recipient =
      to || convo.participants.find((p) => String(p) !== String(userId));
...
const markRead = async (req, res) => {
  try {
    ...
    await Message.updateMany(
      { conversationId, to: userId, readAt: null },
      { $set: { readAt: new Date() } }
    );
